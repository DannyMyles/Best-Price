import Link from "next/link";
import Image from "next/image";
import { ArrowRight } from "lucide-react";
import { ScrollReveal } from "@/components/ui/ScrollReveal";

const banners = [
  {
    eyebrow: "Thin. Light. Powerful.",
    title: "MacBook Air M4",
    subtitle: "All-day battery life in a strikingly thin design.",
    cta: "Shop MacBook Air",
    href: "/products?category=macbook-air",
    image:
      "https://images.unsplash.com/photo-1628115367019-607aedaee62a?q=80&w=1400&auto=format&fit=crop",
  },
  {
    eyebrow: "Work your way",
    title: "Microsoft Surface",
    subtitle: "Laptop performance with tablet flexibility.",
    cta: "Shop Surface",
    href: "/products?category=surface",
    image:
      "https://images.unsplash.com/photo-1665686310429-ee43624978fa?q=80&w=1400&auto=format&fit=crop",
  },
];

export function PromoBanners() {
  return (
    <section className="mx-auto max-w-[1600px] px-4 pb-16 sm:px-6 sm:pb-20 lg:px-8">
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
        {banners.map((b, i) => (
          <ScrollReveal key={b.title} delay={i * 0.1}>
            <Link href={b.href} className="group block">
              <div className="relative h-64 overflow-hidden rounded-3xl bg-surface-muted sm:h-72">
                <Image
                  src={b.image}
                  alt={b.title}
                  fill
                  unoptimized
                  sizes="(min-width: 768px) 50vw, 100vw"
                  className="object-cover transition-transform duration-500 group-hover:scale-105"
                />
                <div className="absolute inset-0 bg-gradient-to-r from-panel-dark/90 via-panel-dark/40 to-transparent" />
                <div className="absolute inset-y-0 left-0 flex max-w-xs flex-col justify-center gap-2 px-7 sm:px-9">
                  <span className="text-xs font-medium text-brand-2">{b.eyebrow}</span>
                  <h3 className="text-2xl font-semibold text-white sm:text-3xl">
                    {b.title}
                  </h3>
                  <p className="text-sm text-white/70">{b.subtitle}</p>
                  <span className="mt-3 inline-flex w-fit items-center gap-1.5 rounded-full bg-white px-4 py-2 text-sm font-medium text-ink transition-transform group-hover:translate-x-1">
                    {b.cta} <ArrowRight className="h-4 w-4" />
                  </span>
                </div>
              </div>
            </Link>
          </ScrollReveal>
        ))}
      </div>
    </section>
  );
}
